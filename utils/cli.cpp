...
CommandLineInterface::CommandLineInterface(CmdHandler fallbackFunction) : fallback(std::move(fallbackFunction)), running(false) {}

void CommandLineInterface::addCommand(const std::string &command, CmdHandler handler) {
...
void CommandLineInterface::exit() { running = false; }

void CommandLineInterface::setPrompt(const std::string &prompt) {
    this->prompt = prompt;
}
